...
public class PatientTriage {
  ...
  /**
    ...
    //implement this method
    		...
    		Patient timeFirst = timeHeap.peek();
    		Time wait = timeFirst.getArrivalTime().elapsed(currentTime);
    		
    		TimeComparator timeCompare = new TimeComparator();
    		
    		if (timeCompare.compare(wait, getMaxWait()) > 0) {
    			timeHeap.poll();
    			priorityHeap.remove(timeFirst.getPriorityPos());
    			return timeFirst;
    		}
    		
    		Patient priorityFirst = priorityHeap.peek();
    		
    		priorityHeap.poll();
    		timeHeap.remove(priorityFirst.getTimePos());
